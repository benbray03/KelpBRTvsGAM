...
giant_se_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    giant_model <- read_csv("data/giant_se_future_predictions.csv")
    
    mpa <- st_read("data/California_Marine_Protected_Areas_[ds582].shp") %>%
      st_transform(crs = 4326) %>%
      mutate(centroid = st_centroid(geometry)) %>%
      filter(st_coordinates(centroid)[, 1] > -120)
    ...
    all_palettes <- list(
      ...
      no3_deepest_annual = colorNumeric("viridis", domain = all_data$no3_deepest_annual, na.color = "transparent"),
