...
giant_csw_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    giant_csw_model <- arrow::read_parquet("brt_projections_by_rock/giant_csw_future_projections_300res.parquet")
    ...
    # Pass giant_csw_model explicitly to avoid scoping issues
    ...
    giant_csw_by_model <- list(
      gfdl     = process_model_data(giant_csw_model, "gfdl"),
      hadl     = process_model_data(giant_csw_model, "hadl"),
      ipsl     = process_model_data(giant_csw_model, "ipsl")
    )
    
    all_data <- do.call(rbind, giant_csw_by_model)
    ...
    filtered_data <- reactive({
      df <- giant_csw_by_model[[input$model]] %>%
