giant_se_ui <- function(id) {
  ns <- NS(id)
  
  tagList(
    titlePanel("Giant Kelp (South East)"),
    sidebarLayout(
      sidebarPanel(
...
giant_se_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ns <- session$ns
    
    giant_se_model <- arrow::read_parquet("brt_projections_by_rock/giant_se_future_projections_300res.parquet")
    
    mpa <- st_read("shp/California_Marine_Protected_Areas_[ds582].shp") %>%
      st_transform(crs = 4326) %>% 
      mutate(centroid = st_centroid(geometry)) %>%
      filter(st_coordinates(centroid)[, 1] > -120.1 &
               st_coordinates(centroid)[, 2] < 34.5)
    
    # Pass giant_se_by_model explicitly to avoid scoping issues
    ...
    giant_se_by_model <- list(
      gfdl     = process_model_data(giant_se_model, "gfdl"),
      hadl     = process_model_data(giant_se_model, "hadl"),
      ipsl     = process_model_data(giant_se_model, "ipsl")
    )
    
    all_data <- do.call(rbind, giant_se_by_model)
    
    all_palettes <- list(
      macpyr_by_rock             = colorNumeric(viridisLite::turbo(256), domain = all_data$macpyr_by_rock, na.color = "transparent"),
      ...
      temp_surface_upwelling = colorNumeric(viridisLite::turbo(256), domain = all_data$temp_surface_upwelling, na.color = "transparent"),
      urchin_log                 = colorNumeric(viridisLite::turbo(256), domain = all_data$urchin_log, na.color = "transparent")
    )
    
    filtered_data <- reactive({
      df <- giant_se_by_model[[input$model]] %>%
    ...
    output$map <- renderLeaflet({
      leaflet() %>%
        ...
        setView(lng = -119.5, lat = 33.8, zoom = 7) %>%
        addPolygons(
          data = mpa,
          color = "#2b83ba",
          weight = 1,
        ...
        addLayersControl(
          overlayGroups = c("MPAs", "macpyr_tiles"),
