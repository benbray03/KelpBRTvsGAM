giantstipes_se_ui <- function(id) {
  ns <- NS(id)
  
  tagList(
    titlePanel("Giant Kelp Stipes (South East)"),
    sidebarLayout(
      sidebarPanel(
...
giantstipes_se_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ns <- session$ns
    
    giantstipes_se_model <- arrow::read_parquet("brt_projections_by_rock/giantstipes_se_future_projections_300res.parquet")
    ...
    giantstipes_se_by_model <- list(
      gfdl     = process_model_data(giantstipes_se_model, "gfdl"),
      hadl     = process_model_data(giantstipes_se_model, "hadl"),
      ipsl     = process_model_data(giantstipes_se_model, "ipsl")
    )
    
    all_data <- do.call(rbind, giantstipes_se_by_model)
    ...
    filtered_data <- reactive({
      df <- giantstipes_se_by_model[[input$model]] %>%
