giant_5_ui <- function(id) {
  ns <- NS(id)
  
  tagList(
    titlePanel("Giant Kelp"),
    sidebarLayout(
      sidebarPanel(
        ...
        selectInput(ns("variable"), "Select Variable to Map:",
                    choices = c(
                      ...
                      "Temp at Surface Upwelling" = "temp_surface_upwelling",
                      "Depth"                     = "depth",
                      "Urchin Log"                = "urchin_log"
                    ),
                    selected = "macpyr_by_rock"),
        br(), br(),
...
giant_5_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ns <- session$ns
    
    giant_model <- read_csv("brt_projections_by_rock/giant_ensemble_future_projections_300res_5yr.csv")
    ts_data    <- read_csv("brt_projections_by_rock/giant_5yr_barplot.csv")
    ...
    all_palettes <- list(
      macpyr_by_rock = colorNumeric(viridisLite::turbo(256),domain = giant_model$macpyr_by_rock, na.color = "transparent"),
      no3_deepest_summer = colorNumeric(viridisLite::turbo(256), domain = giant_model$no3_deepest_summer, na.color = "transparent"),
      taualong_surface_upwelling = colorNumeric(
        scales::div_gradient_pal(low="#2166ac", mid="white", high="#d73027")(seq(0,1,length=256)),domain = giant_model$taualong_surface_upwelling, na.color = "transparent"),
      taucross_surface_upwelling = colorNumeric(scales::div_gradient_pal(low="#2166ac", mid="white", high="#d73027")(seq(0,1,length=256)), domain = giant_model$taucross_surface_upwelling, na.color = "transparent"
      ),
      temp_deepest_upwelling = colorNumeric(viridisLite::turbo(256),   domain = giant_model$temp_deepest_upwelling, na.color = "transparent"),
      temp_surface_upwelling = colorNumeric(viridisLite::turbo(256),   domain = giant_model$temp_surface_upwelling, na.color = "transparent"),
      depth = colorNumeric(viridisLite::mako(256, direction = -1),    domain = giant_model$depth, na.color = "transparent"),
      urchin_log = colorNumeric(viridisLite::turbo(256),domain = giant_model$urchin_log, na.color = "transparent")
    )
    ...
    output$timeSeriesPlot <- renderPlot({
      ...
      ggplot(ts_data, aes(x = year_bin, y = total_abundance)) +
        ...
        labs(
          title = "Total Giant Kelp Abundance by 5-Year Period",
          x = "Year",
          y = "Total Abundance",
